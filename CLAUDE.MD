# Objective
Build a full-stack calculator application with a React frontend and a backend microservice. The frontend should consume the backend API to perform basic and advanced arithmetic operations. Focus on clean design, maintainable code, and testable architecture.

# Usability Features
- Numeric keyboard for entry
- One number comes from keyboard and the other comes from memory starting with 0
- The confirmation of the first number imputed should be the click of any operator by the user

# Math Operations
- User needs to able to oprate with decimal postive and negative nubers
- Addition
- Subtraction 
- Multiplication 
- Division 
- Exponentiation
- Square Root 
- Percentage

# Frontend (React)
- Intuitive UI for entering input and displaying results
- Input validation and error handling
- Responsive design (basic mobile support)

# Backend (REST API):
- Expose endpoints for calculator operations
- Validate input and handle edge cases (division by zero, invalid data)
- Return results in JSON format

# Non-Functional
- Clean, readable, and idiomatic code (frontend and backend)
- Unit tests covering key functionality for both layers
- Documentation: setup instructions, API usage, and design rationale
- Dockerfile for full-stack deployment

# Constraints
- Frontend: React (TypeScript preferred)
- Backend: Go is perferred

# Deliverables
- Git repository with frontend and backend code
- README with setup instructions, API examples, and design decisions
- Unit tests and coverage report
- Dockerfile to run frontend + backend together

# Notes
Create a repo in my github (already logged in the terminal)
The repo will be called go-calculator


# Prompts
All the prompts used for working pourposes must be saved to PROMPTS.MD

# Visuals
Design that closely resembles iphone calculator for both light and dark theme.
Need a toggle switch to change between the both teams but use the default theme as innitial state for it.